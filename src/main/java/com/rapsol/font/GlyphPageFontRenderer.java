...
public final class GlyphPageFontRenderer {
	...
	public static GlyphPageFontRenderer createFromID(CharSequence id, int size, boolean bold, boolean italic,
		...
		//built in font instead of loading a TTF
		...
		if (bold) {
			boldPage = new GlyphPage(new Font(Font.SANS_SERIF, Font.BOLD, size), true, true);
		...
		if (italic) {
			italicPage = new GlyphPage(new Font(Font.SANS_SERIF, Font.ITALIC, size), true, true);
		...
		if (boldItalic) {
			boldItalicPage = new GlyphPage(new Font(Font.SANS_SERIF, Font.BOLD | Font.ITALIC, size), true, true);
	...
	private void renderStringAtPos(GuiGraphicsExtractor context, CharSequence text, boolean shadow, int color) {
		...
		for (int i = 0; i < text.length(); ++i) {
			...
			if (c0 == 167 && i + 1 < text.length()) {
				...
				float f = glyphPage.drawChar(context, c0, posX, posY, g, k, h, alpha);

				doDraw(context, f, glyphPage);
			}
		}
	...
	private void doDraw(GuiGraphicsExtractor context, float f, GlyphPage glyphPage) {
		if (this.strikethroughStyle) {
			context.fill((int) this.posX, (int) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2) - 1.0F), (int) (this.posX + f), (int) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2)), 0xFFFFFFFF);
		}

		if (this.underlineStyle) {
			int l = this.underlineStyle ? -1 : 0;
			context.fill((int) (this.posX + (float) l), (int) (this.posY + (float) glyphPage.getMaxFontHeight() - 1.0F), (int) (this.posX + f), (int) (this.posY + (float) glyphPage.getMaxFontHeight()), 0xFFFFFFFF);
		}
	...
	public CharSequence trimStringToWidth(CharSequence text, int maxWidth, boolean reverse) {
		...
		return printString.of(stringbuilder.toString());
	}
}
