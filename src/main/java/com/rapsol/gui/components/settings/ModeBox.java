...
public final class ModeBox extends RenderableSetting {
	...
	public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
		...
		TextRenderer.text(setting.getName() + ": ", context, nameOffset, (parentY() + parentOffset() + offset) + 9, new Color(245, 245, 245, 255).getRGB());
		nameOffset += TextRenderer.getWidth(setting.getName() + ": ");

		int modeOffset = nameOffset;

		TextRenderer.text(setting.getMode().name(), context, modeOffset, (parentY() + parentOffset() + offset) + 9, new Color(245, 245, 245, 255).getRGB());
